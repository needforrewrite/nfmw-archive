...
impl User {
    pub fn new_local_from_password(
        username: String,
        password: String,
    ...
    pub async fn create_oauth(pool: &sqlx::PgPool, username: &str) -> Result<i32, sqlx::Error> {
    ...
    pub async fn get_by_username_and_local_password(
        pool: &sqlx::PgPool,
        username: &str,
        password: &str,
    ) -> Result<Option<User>, sqlx::Error> {
        if let Some(user) = User::get_by_username(pool, username).await? {
            if let Some(ref hash) = user.phash.clone()
                && let Some(ref salt) = user.psalt
            {
